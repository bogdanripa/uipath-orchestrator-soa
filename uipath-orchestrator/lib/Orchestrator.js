...
var CLOUD_PLATFORM = {
    ...
    platformHostname: 'cloud.uipath.com'
};
...
function validateOptions(options) {
    ...
    if (options.refreshToken && !options.hostname) {
        options.hostname = CLOUD_PLATFORM.platformHostname;
    }    
    if (!options.serviceInstanceLogicalName && options.tenancyName) {
        options.serviceInstanceLogicalName = options.tenancyName;
    }
    return options;
}
...
Orchestrator.prototype._processElement = function (element, isRetry) {
    ...
    if (element.organizationUnitId !== -1) {
        options.headers[ORGANIZATION_UNIT_ID_HEADER] = element.organizationUnitId;
    }
...
Orchestrator.prototype._methodWrapper = function (method, path, data, callback) {
    ...
    this._queue.push({
        ...
        organizationUnitId: this._activeOrganizationUnitId,
        data: data,
        cb: callback
    });
    this._processQueue();
